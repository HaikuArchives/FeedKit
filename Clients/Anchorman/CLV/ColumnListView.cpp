...
void BRow::MouseDown(BColumnListView *parent, BField *field, BPoint point, uint32 buttons) {
	fprintf(stderr, "BRow::MouseDown([%p], [%p], (%.2f, %.2f), %i) called\n", parent, field, point.x, point.y, buttons);
};

void BRow::MouseMoved(BColumnListView *parent, BField *field, BPoint point, uint32 buttons, int32 code) {
	fprintf(stderr, "BRow::MouseMoved([%p], [%p], (%.2f, %.2f), %i, %i) called\n", parent, field, point.x, point.y, buttons, code);
};

void BRow::MouseUp(BColumnListView *parent, BField *field) {
	fprintf(stderr, "BRow::MouseUp([%p], [%p]) called\n", parent, field);
};
...
BColumn::BColumn(BMessage *archive) {
	archive->FindFloat("_width", &fWidth);
	archive->FindFloat("_minwidth", &fMinWidth);
	archive->FindFloat("_maxwidth", &fMaxWidth);
	archive->FindBool("_visible", &fVisible);
	archive->FindInt32("_fieldid", &fFieldID);
	archive->FindBool("_sortascending", &fSortAscending);
	archive->FindBool("_wantsevents", &fWantsEvents);
	archive->FindBool("_showheading", &fShowHeading);
	archive->FindInt32("_alignment", (int32 *)&fAlignment);
};
...
status_t BColumn::Archive(BMessage *archive, bool deep = true) const {
	archive->AddString("class", "BColumn");
	archive->AddFloat("_width", fWidth);
	archive->AddFloat("_minwidth", fMinWidth);
	archive->AddFloat("_maxwidth", fMaxWidth);
	archive->AddBool("_visible", fVisible);
	archive->AddInt32("_fieldid", fFieldID);
	archive->AddBool("_sortascending", fSortAscending);
	archive->AddBool("_wantsevents", fWantsEvents);
	archive->AddBool("_showheading", fShowHeading);
	archive->AddInt32("_alignment", (int32)fAlignment);
	
	return B_OK;
};

BArchivable *BColumn::Instantiate(BMessage *archive) {
	BArchivable *instance = NULL;
	
	if (validate_instantiation(archive, "BColumn") == true) {
		instance = new BColumn(archive);
	};
...
status_t BColumnListView::Archive(BMessage *archive, bool deep = true) const {
	status_t result = BView::Archive(archive, false);
	BColumnListView *that = (BColumnListView *)this;
	
	BMessage state;
	that->SaveState(&state);
	
	archive->AddMessage("state", &state);
	archive->AddInt32("borderStyle", fBorderStyle);
	archive->AddBool("showHorizontalScrollbar", fHorizontalScrollBar->IsHidden() == false);
	if (fSelectionMessage) archive->AddMessage("selectionMessage", fSelectionMessage);
	if (InvocationMessage() != NULL) archive->AddMessage("invocationMessage", InvocationMessage());
	
	for (int32 i = 0; i < CountColumns(); i++) {
		BMessage colArchive;
		BColumn *col = ColumnAt(i);
		
		if (col->Archive(&colArchive, deep) == B_OK) {
			archive->AddMessage("column", &colArchive);
		};
	};
...
BArchivable *BColumnListView::Instantiate(BMessage *archive) {
	BArchivable *instance = NULL;
	
	if (validate_instantiation(archive, "BColumnListView") == true) {
		BRect rect;
		const char *name;
		int32 resizingMode;
		int32 drawFlags;
		...
		archive->FindRect("_frame", &rect);
		archive->FindString("_name", &name);
		archive->FindInt32("_resize_mode", &resizingMode);
		archive->FindInt32("_flags", &drawFlags);
		archive->FindInt32("borderStyle", (int32 *)&border);
		archive->FindBool("showHorizontalScrollbar", &showHorizontalScrollbar);
		archive->FindMessage("state", &state);
		archive->FindMessage("selectionMessage", &selectionMessage);
		archive->FindMessage("invocationMessage", &invocationMessage);
		
		BColumnListView *bclv = new BColumnListView(rect, name, resizingMode, drawFlags, border, showHorizontalScrollbar);
		bclv->SetSelectionMessage(new BMessage(selectionMessage));
		bclv->SetInvocationMessage(new BMessage(invocationMessage));
		
		BMessage colArchive;
	
		for (int32 i = 0; archive->FindMessage("column", i, &colArchive) == B_OK; i++) {
			BColumn *col = dynamic_cast<BColumn *>(instantiate_object(&colArchive));
			bclv->AddColumn(col, col->LogicalFieldNum());
			
			colArchive.MakeEmpty();
		};
		bclv->LoadState(&state);
...
//						}
		...
		if(handle_field) {
			...
			fCurrentRow->MouseDown(fMasterView, fCurrentField, position, buttons);
		}
...
//					fCurrentRow->MouseMoved(fMasterView, fCurrentField, position, 0, fCurrentCode = B_EXITED_VIEW);
				}
				fCurrentColumn = new_column;
				fCurrentRow = new_row;
				fCurrentField = new_field;
...
//					fCurrentRw->MouseMoved(fMasterView, fCurrentField, position, 0, fCurrentCode = B_ENTERED_VIEW);
				}
			} else {
				if(fCurrentField) {
					fCurrentColumn->MouseMoved(fMasterView, fCurrentRow, 
						fCurrentField, fFieldRect, position, 0, fCurrentCode = B_INSIDE_VIEW);
//					fCurrentRow->MouseMoved(fMasterView, fCurrentField, position, 0, fCurrentCode = B_INSIDE_VIEW);
				}
			}
		} else {
			if(fCurrentField) {
...
void OutlineView::MouseUp(BPoint position)
{
	if(fCurrentField) {
		...
		fCurrentRow->MouseUp(fMasterView, fCurrentField);
		fMouseDown = false;
	}
